...
date: 2026-07-19
description: The concepts that carry every concurrency interview, atomicity, visibility, ordering, the Java Memory Model, the condition loop, semaphores, deadlock, executors, reading code for bugs, and what changes when concurrency leaves the JVM.
...
Companion pages: [the interview framework](/interview/multithreading/mt-framework/) for the method, and the [pattern playbooks](/interview/multithreading/) for the ten families.
...
## 6. The ten families — essence, mechanic, signature bug
...
| 8 | **Concurrent data structures** | family 2 with a throughput requirement — does the invariant *decompose*? | lock striping / lock-free CAS / immutable snapshot, chosen by which invariant clauses are local | contended global clause (LRU recency list); ABA; false sharing |
| 9 | **Debugging & code review** | run the method backwards: symptom → cause | five-sweep scan (shared state, check-then-act, wait/signal, lock order, lifecycle) + reproduction | diagnosing without reproducing; fixing symptoms not causes |
| 10 | **Distributed concurrency** | the same problems with no shared memory and partial failure | relocate each primitive: mutex→lease+fencing, CAS→version, `putIfAbsent`→unique constraint | non-fenced expired lock; retry storms; dedup window too small |
...
16. **Invariant decomposition** (the data-structure move): write the invariant clause by clause and test each for *locality*. Clauses about one key shard beautifully (hash map → per-bin locks); clauses about a **global ordering** (LRU recency, a FIFO guarantee) do not — that clause is your serialization point. This test tells you in thirty seconds whether striping is available.
17. **Renegotiating the spec**: when a global clause blocks you, the senior move is to ask whether the *requirement* can bend — approximate LRU, estimated `size()`, weakly-consistent iteration, lossy recency buffers. Relaxing exactness to buy concurrency is a legitimate design act, not cheating; name the guarantee you are giving up.
18. **Per-key striping**: one lock/queue/limiter *per key* rather than one global — the same move behind per-client rate limiters, per-key cached futures, per-key ordered event buses, and CHM's bins. Different keys never contend; same key serializes. The generic answer to "make it scale without losing per-entity ordering".
19. **Lock-free / CAS discipline**: read snapshot → compute successor → compareAndSet → retry. Progress guarantees are a ladder: wait-free (every thread finishes in bounded steps) > lock-free (*some* thread always progresses) > obstruction-free > blocking. **ABA**: the value returned to A while the world changed underneath — fixed with version stamps. Lock-free wins under moderate contention on small state; under heavy contention the retry waste can lose to a plain mutex. Measure, don't assume.
20. **Mechanical sympathy**: correctness isn't the only cost. **False sharing** (two unrelated variables on one cache line ping-ponging between cores) and contended-CAS retry waste are invisible to the memory model and to every correctness argument — they're why `LongAdder` beats `AtomicLong` on hot counters (per-cell accumulation, sum on read: the sharding trade applied to a single number).
21. **Idempotency**: make the operation safe to repeat, because at-least-once delivery plus retries means it *will* repeat. A client-supplied key plus a uniqueness constraint turns "did this already happen?" into an atomic claim — the distributed twin of `putIfAbsent`, with the constraint as the linearization point. Exactly-once *delivery* is impossible; exactly-once *effects* are achievable.
22. **Fencing tokens**: a lease is not a mutex. A lock with a TTL can expire while its holder is still working (GC pause, slow disk), so two holders can believe they own it. The fix is not a longer TTL — it's a monotonically increasing token passed to the resource, which rejects any write carrying a stale token. Any distributed-lock answer without fencing is incomplete.
23. **Optimistic vs pessimistic**: pessimistic takes the lock first (`SELECT … FOR UPDATE`) — right under high contention; optimistic reads a version, computes, and commits conditionally on the version being unchanged, retrying on conflict — right under low contention. That's CAS, relocated to a database. Both re-check under the same rule you already know: never act on a value you read outside the guard.
...
## 11b. Reading code for bugs, and proving the fix (concepts: diagnosis, thread states, reproduction)

A whole interview round at some companies is "here is code that already works most of the time — find the bug." It runs backwards from everything else in this document, so it needs its own method.

**The five-sweep scan** — read the code five times, once per question, instead of once looking for "anything wrong":

1. What is the shared mutable state, and what guards each piece?
2. Where is every check-then-act (including `count++` and derive-then-act)?
3. Where is every wait/signal pair — `while` or `if`? `notify` or `notifyAll`? Is the state change under the same lock?
4. What is every lock-acquisition order, across *all* code paths? (Inversions are only visible when you compare two methods.)
5. What are the lifecycle paths — exception, cancellation, shutdown, timeout? Is every release in a `finally`?

**Symptom → cause, the fast lookup:** counts/balances drift under load → unguarded read-modify-write. Hangs with a lock cycle in the dump → deadlock. Hangs with *no* cycle → lost wakeup, or waiting on work that will never arrive. Works in dev, fails in prod/after JIT warmup → visibility or reordering (missing `volatile`/unsafe publication). Passes 999 runs, fails the 1000th → a race whose window is small, not absent. Throughput silently decays to zero → workers died to uncaught exceptions.

**Thread dumps:** `BLOCKED` = waiting for a monitor another thread holds (deadlock candidate — the dump names the owner). `WAITING`/`TIMED_WAITING` = parked in `wait`/`await`/`park`, i.e. waiting for a *signal* — no cycle will appear, which is exactly the signature of a lost wakeup. Take three dumps a few seconds apart: threads stuck in the same frame across all three are genuinely stuck rather than merely busy.

**Reproduction is the deliverable, not the fix.** A race you cannot reproduce is a race you cannot prove you fixed. Convert probabilistic to deterministic: a `CountDownLatch` start gate so threads collide, injected delays at the suspect interleaving point to widen the window, many iterations, invariant assertions, and timeouts so a hang fails the test instead of hanging CI. Then: **a green stress test is not evidence** — the correctness argument is. Fix causes, not symptoms, and rank findings by blast radius when reviewing.

## 11c. When concurrency leaves the JVM (concepts: partial failure, relocation)

The standard senior follow-up is "now run this on fifty hosts." Three things break that your single-JVM instincts depend on:

- **No shared memory** — no lock, no `volatile`, no happens-before between hosts. Coordination must travel through a store or a message.
- **Partial failure** — a timeout means *"I don't know whether it happened"*, a state that has no single-JVM equivalent. This one fact generates the entire idempotency discipline.
- **No failure atomicity and no trustworthy clock** — there is no `finally` across hosts (hence TTLs everywhere), and wall clocks on different machines are incomparable while pauses are unbounded (hence fencing rather than time-based reasoning).

**The translation table** — every distributed mechanism is a primitive you already own, relocated:

| Single JVM | Distributed |
|---|---|
| `synchronized` / mutex | lease with TTL **+ fencing token** (not a mutex) |
| CAS / `compareAndSet` | conditional write: version in the update predicate (OCC) |
| `putIfAbsent` | unique constraint / conditional insert — the claim's linearization point |
| `AtomicLong` counter | DB atomic increment, or sharded/bucketed counters |
| condition wait | polling, queues, or change notifications |
| happens-before | per-partition/per-key ordering guarantees, causality tokens |
| `finally` cleanup | TTL + expiry reconciliation |

**Say the single-JVM answer first, cold, with its linearization point named — then relocate it.** That ordering is itself graded.
...
22. ABA — a CAS succeeds because the value matches, though the world changed underneath (fix: version stamps)
23. False sharing — unrelated variables sharing a cache line; correct but slow, and invisible to every correctness argument
24. Sharding a global clause — striping a structure whose invariant has a non-local clause (LRU recency, FIFO), producing a correct-looking structure that still serializes or silently loses the ordering guarantee
25. Non-fenced expired lease — TTL expires during a pause, two holders both believe they own it, and the resource accepts both writes
26. Retry storms — synchronized exponential backoff with no jitter re-converges the fleet into a thundering herd
27. Dedup window too small — an idempotency/dedup record expires before the retry that needs it arrives
28. Clock-based reasoning across hosts — comparing wall clocks, or assuming a pause is bounded
29. Non-idempotent retries — retrying an operation that isn't safe to repeat, so at-least-once delivery double-charges
