...
The recipe applied to every problem in this category, then to two transfer targets that reference this pattern. (Recipe note: the first draft of the recipe began at R1; validating against the read-heavy cache showed it over-applied the lightswitch to a problem whose best read path has no lock at all. R0 was added and all validations re-run, they pass as written above.)

### 7.1 reader-writer-lock (primary)
...
R0: doesn't fire, occupancy is a physical exclusion, not a data read, so there is no lock-free bypass. R1: matrix, man/man ✓, woman/woman ✓, man/woman ✗; **two internally-compatible categories**, which is the case reader-writer-lock cannot exercise because its writers are internally exclusive. R2: invariant "occupants are all of one category". R3 does the work: two lightswitches (one counter+mutex per category) wired to one `roomEmpty`, the recipe's "each internally-shareable category holds its rooms via a lightswitch" clause, applied twice. R4: readers-preference generalizes to "whichever category got there first keeps winning" ⇒ each side starves the other; turnstile fix, and F5's "don't call it fair" applies verbatim. R5 **fires for the first time in this family**: the capacity limit is an orthogonal multiplex acquired per-member inside the room, validating that R5 belongs in the recipe rather than being folded into R3. R6: F1 (torn counter → two "first" arrivals), F2 (missing last-out release), F6 (owner-checked room lock) are exactly this problem's pitfalls 2, 3, 1. **Verdict: PASS, and it is the problem that exercises R5 and the two-lightswitch clause of R3, both of which the primary problem leaves untested.**

### 7.3 search-insert-delete

R0: doesn't fire. R1 is the entire problem: the 3×3 matrix is **asymmetric**, insert/search ✓ but insert/insert ✗, which no amount of pattern-matching on "readers and writers" will produce. R2: invariant read off the matrix. R3 exercises all three of its clauses at once, which nothing else in the family does: searchers internally compatible ⇒ lightswitch on `noSearcher`; inserters internally exclusive ⇒ acquire `noInserter` **directly** (a mutex is a degenerate lightswitch, max count one); deleters need both rooms ⇒ **fixed acquisition order**, which is R3's final clause and the family's only collision with multi-lock deadlock. R4: deleters starve first; same turnstile. R6: F1, F2, F6 apply; the deleter's block-while-holding needs the dependency argument from §1, which holds because inserters never want `noSearcher`. **Verdict: PASS. This is the general case, the recipe derives it mechanically from the matrix, which is the strongest evidence that R1 (draw the matrix first) is correctly placed before R2 rather than after.**

### 7.4 Transfer: traffic-light intersection, parallel-crossing variant
...
### 7.5 Transfer: read-heavy cache with expiry, read path
...
Does the 5-step framework (the [framework page](/interview/multithreading/mt-framework/)) suffice for this category? Mostly, Step 1 Type E, Step 3 pattern 6, and Step 5's starvation check (item 4, which even names "writers behind endless readers") carry the core. Three gaps:
...
3. **Minor: no Type E template.** Step 4 gives memorizable skeletons for Types A–D; Type E has none. The two skeletons in section 2 of this document are the missing Template 5.
