...
**Trade-offs**: beautiful symmetry, permits persist (a release before anyone waits is never lost — no lost-wakeup class of bugs), and it composes naturally with special-role signaling (Section 5). Cost: the count now lives in the semaphores, so a compound predicate ("empty AND shop open") doesn't fit — you fall back to A/B, or bolt a guarded counter alongside (as barbershop does). Also carries the strict ordering rule of Section 8.

**Choosing**: asked to hand-build with wait/notify → A. Two-plus distinct waiter kinds and free choice of tools → B. Signaling between distinct roles, rendezvous, or "the count IS the resource" → C. In a design round: none of the above — `ArrayBlockingQueue` and say so (anti-over-engineering rule 7).
