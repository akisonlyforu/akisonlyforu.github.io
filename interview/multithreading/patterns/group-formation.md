...
4. **Peers or coordinator?** Problem gives a dedicated service thread with its own loop → Shape C. Otherwise symmetric peers. (If both a coordinator *and* multiple types with choice appeared — rare — the coordinator runs the dispatcher logic inside its loop: tallies feed the coordinator instead of a hat.)
