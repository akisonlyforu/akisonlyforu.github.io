...
- Follow-up: `thenApply(Function<T,R>)` registers a callback that runs when the value arrives (returning a new MyPromise).
...
- **timed get()**: `wait(remaining)` in the loop, recomputing remaining from `nanoTime` each wake (three wake reasons, timeout, notify, and spurious, all get identical treatment: re-check; this is the timed-wait discipline from the scheduler, in miniature). On expiry with status still PENDING → throw TimeoutException.
