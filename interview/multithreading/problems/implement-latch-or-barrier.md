...
The latch is easy; the barrier is not — and the gap BETWEEN them is the lesson. Reusability forces the "generation" idea (how does a thread know the wait it's in belongs to round k, not round k+1?), which is the same reasoning as the Little Book of Semaphores's two-turnstile reusable barrier and the reason [Template 4](/interview/multithreading/mt-framework/) uses CyclicBarrier for H2O. Build the latch, then attempt the barrier, then read about generations.
