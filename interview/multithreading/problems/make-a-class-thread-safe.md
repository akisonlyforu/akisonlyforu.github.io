...
Guarded state. Invariant depends on the class, so state it for THIS class before touching code (e.g., stack: `0 <= size <= capacity`, elements LIFO, no lost or duplicated element).
...
- **`size()` semantics**: under concurrency, a size is a snapshot, stale the moment it returns. Callers must not treat it as a guarantee, which connects directly back to the compound-op trap.
